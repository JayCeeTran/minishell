...
char	*ft_strjoin_free(char *s1, char *s2)
{
	...
	res = ft_strjoin(s1, s2); // malloc fail needed
	free(s1);
	return (res);
}
...
void	append_char(char **dst, char c)
{
	...
	buf[0] = c;
	buf[1] = '\0';
	*dst = ft_strjoin_free(*dst, buf); // add malloc fail!
}
	...
	while (input[i])
	{
		...
		else if (input[i] == '$' && !in_single && input[i + 1])
		{
			...
			while (input[i] && is_valid_var_char(input[i]))
				i++;
			char	*key = ft_substr(input, start, i - start); //malloc fail handling needed!
			char	*val = get_env_value(key, envp);
			result = ft_strjoin_free(result, val); //here too!
			free(key);
			i--;
		}
		else
