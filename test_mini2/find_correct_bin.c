...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/26 19:24:20 by jtran             #+#    #+#             */
/*   Updated: 2025/08/27 16:54:58 by jtran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*find_bin(t_cmd *cmd, t_data *data)
{
	...
	{
		is_path_valid(data, cmd->cmd[0], 0, 0);
		if (access(cmd->cmd[0], F_OK) == 0)
		{
			if (access(cmd->cmd[0], X_OK) == 0)
			{
				...
				error_msg_exit(1, data, cmd->cmd[0]);
		}
		else
			error_msg_exit(0, data, cmd->cmd[0]);
	}
